...
import com.example.kuoti.atk.enums.Colors
import com.example.kuoti.atk.model.Item
import com.example.kuoti.atk.util.fileToImageView
import com.example.kuoti.atk.util.getUniqueFileName
import java.io.File
import java.util.*
...
class ItemActivity : Activity() {
    ...
    //check if set background color or not when create note
    private var setBackgroundColor : Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 如果是修改記事
        if (intent.action == "com.example.kuoti.atk.EDIT_ITEM") {
            // 接收與設定記事標題
            item = intent.extras.getSerializable("com.example.kuoti.atk.model.Item") as Item
    ...
    private fun takePicture() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            ...
            uri = FileProvider.getUriForFile(this,
                     "com.example.kuoti.atk.provider",
                    file)
        }
        else {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            startActivity(intent,
                    ActivityOptions.makeSceneTransitionAnimation(
                            this@ItemActivity, picture, "picture").toBundle())
        } else {
            startActivity(intent)
        }
    }
