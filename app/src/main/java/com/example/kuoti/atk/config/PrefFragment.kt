package com.example.kuoti.atk.config
...
import com.example.kuoti.atk.activity.ItemActivity
...
class PrefFragment : PreferenceFragment(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private fun setColorSummary() {
        ...
        if (color != -1) {
            // 設定顏色說明
            defaultColor.summary = getString(R.string.default_color_summary) + ": " + ItemActivity.getColors(color)
            //defaultColor.setSummary(color)
        }
    }
